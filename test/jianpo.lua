...
function GetSkillLevelData(skill)
	...
	skill.AddAttribute(
		...
		tSkillData[dwSkillLevel].nDamage / 2 + 10,
		dwSkillLevel
	);
	...
	skill.AddAttribute(
		...
		tSkillData[dwSkillLevel].nDamageRand / 2 + 10,
		dwSkillLevel
	);
 ...
 - 4, -- 属性值1
	...
	skill.AddSlowCheckSelfBuff(409, 5, BUFF_COMPARE_FLAG.GREATER_EQUAL, 0, BUFF_COMPARE_FLAG.GREATER_EQUAL);
	...
	if dwSkillLevel < 10 then
		skill.nChannelInterval = 80 * 1.1 * 1.1* 1.1;
	elseif dwSkillLevel < 18 then
		skill.nChannelInterval = (80 + (dwSkillLevel - 9) * 15) * 1.1 * 1.1* 1.1
	else
		skill.nChannelInterval = 220 * 1.1 * 1.1* 1.1;     -- 通道技间隔时间
	end
...
function CanCast(player, nPreResult,SkillTarget)
--判断玩家的状态，以判断是否可以发出技能
	
	if nPreResult == SKILL_RESULT_CODE.SUCCESS then
		--凤吟非侠士目标
		--local targetType = SkillTarget.GetTargetType()
		--if player.GetSkillLevel(5855) == 1 then
			
			--if targetType == TARGET.NPC then
				
				--player.AddBuff(player.dwID, player.nLevel, 24632, 1)
			--end
		--end
		return nPreResult;
	end
...
function Apply(dwCharacterID)
	...
	if player.GetBuff(1917, 3) or player.GetBuff(1917, 4) or player.GetBuff(1917, 5)then --90级橙武
		player.AddBuff(dwCharacterID, player.nLevel, 6469, 1, 1)
	end
	--打一次破招
	if player.nSurplusValue ~= 0 then
		player.CastSkill(32957,1)
	end
	
	
	if player.GetSkillLevel(5868) == 1 then
		player.nCurrentMana = player.nCurrentMana + 0.1 * player.nMaxMana
	end
	--小队叠加破防
